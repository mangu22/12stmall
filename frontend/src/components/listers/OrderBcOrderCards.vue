    ...
    <div>
        ...
        >
            <v-row>
                <v-list-item class="d-flex" style="background-color: white;">
                    ...
                    <div class="secondary-text-color" style="margin-left:30px;">주문테이블? jpa에 Entity단위라고 생각을 하면..?</div>
                </v-list-item>
            </v-row>
        </v-card>
        <v-col style="margin-bottom:40px;">
            <div class="text-center">
                ...
                >
                    <OrderBcOrder :offline="offline" class="video-card" :isNew="true" :editMode="true" v-model="newValue" 
        ...
        <v-row>
            <OrderBcOrder :offline="offline" class="video-card" v-for="(value, index) in values" v-model="values[index]" v-bind:key="index" @delete="remove"/>
        </v-row>
    </div>
</template>
...
<script>
    ...
    import OrderBcOrder from './../OrderBcOrder.vue';

    export default {
        name: 'OrderBcOrderManager',
        components: {
            OrderBcOrder,
        },
        props: {
